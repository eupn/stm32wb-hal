...
impl LpTimer<Periodic> {
    ...
    pub fn init_periodic(lptim: LPTIM1, rcc: &mut Rcc) -> Self {
        Self::init(lptim, rcc)
    }
}
...
impl LpTimer<OneShot> {
    ...
    ///
    /// The timer needs to be started by calling `.start(freq)`.
    pub fn init_oneshot(lptim: LPTIM1, rcc: &mut Rcc) -> Self {
        Self::init(lptim, rcc)
    }
}

impl<M: CountMode> LpTimer<M> {
    fn init(lptim: LPTIM1, rcc: &mut Rcc) -> Self {
        let input_freq = rcc.clocks.lptim1();
